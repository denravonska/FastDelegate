...
struct SimplifyMemFunc<SINGLE_MEMFUNCPTR_SIZE>  {
   ...
   inline static GenericClass *Convert(X *pthis, XFuncType function_to_bind,
         GenericMemFuncType &bound_func) {
#if defined __DMC__ || defined(__ARMEL__)
...
public:
   ...
   inline GenericMemFunc GetClosureMemPtr() const { return horrible_cast<GenericMemFunc>(m_pFunction); }
...
public:
   ...
   typedef FastDelegateN type;

   // Construction and comparison functions
   FastDelegateN() { clear(); }
   FastDelegateN(const FastDelegateN &x) {
      m_Closure.CopyFrom(this, x.m_Closure); }
   void operator = (const FastDelegateN &x)  {
      m_Closure.CopyFrom(this, x.m_Closure); }
   bool operator ==(const FastDelegateN &x) const {
   ...
   FastDelegateN(Y *pthis, DesiredRetType (X::* function_to_bind)(Args... args) ) {
   ...
   template < class X, class Y >
   FastDelegateN(const Y *pthis, DesiredRetType (X::* function_to_bind)(Args... args) const) {
   ...
   FastDelegateN(DesiredRetType (*function_to_bind)(Args... args) ) {
   ...
   inline void bind(DesiredRetType (*function_to_bind)(Args... args)) {
      m_Closure.bindstaticfunc(this, &FastDelegateN::InvokeStaticFunction,
         function_to_bind); }
   // Invoke the delegate
   RetType operator() (Args... args) const {
   return (m_Closure.GetClosureThis()->*(m_Closure.GetClosureMemPtr()))(args...); }
   // Invoke the delegate if assigned. This will not return a value.
   RetType invoke_if(Args... args) {
      return !empty()
            ? (m_Closure.GetClosureThis()->*(m_Closure.GetClosureMemPtr()))(args...)
            : RetType(); }
...
// FastDelegate< R ( ... ) >
// instead of
// FastDelegateN < R, Param1 >
template<typename R, typename... Args>
class FastDelegate< R ( Args... ) >
  // Inherit from FastDelegateN so that it can be treated just like a FastDelegateN
  : public FastDelegateN < R, Args... >
{
public:
  // Make using the base type a bit easier via typedef.
  typedef FastDelegateN < R, Args... > BaseType;
...
FastDelegateN<FASTDLGT_RETTYPE, Args...> MakeDelegate(Y* x, RetType (X::*func)(Args... args)) {
   return FastDelegateN<FASTDLGT_RETTYPE, Args...>(x, func);
}

template <class X, class Y, class RetType, typename... Args>
FastDelegateN<FASTDLGT_RETTYPE, Args...> MakeDelegate(Y* x, RetType (X::*func)(Args... args) const) {
   return FastDelegateN<FASTDLGT_RETTYPE, Args...>(x, func);
}
...
void invoke_if(const T& delegate, Args... args) {
   if(!delegate.empty())
      delegate(args...);
}
